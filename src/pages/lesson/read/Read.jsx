...
const Read = ({ lesson }) => {
  return (
    <MySection>
      <p className={`${outerCl.suptitle} text text-12 color-gray`}>Самая Вышка!</p>
      <h1 className={`title title-page`}>
        Урок {lesson}. Где я и куда идти?
      </h1>
      <img className={outerCl.img + ' br-6 w-full'} src={img} alt="lesson img" />
      ...
      <Link className={`${outerCl.btn} btn btn-bg max-w-280 m-x-auto fz-14`} >Начать</Link>
    </MySection>
  );
};
